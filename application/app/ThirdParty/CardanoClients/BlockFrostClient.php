...
class BlockFrostClient implements ICardanoClient {
    ...
    public function getAssetMetadata(string $assetId): ?array {
        return $this->call(self::HTTP_REQUEST_GET, "assets/{$assetId}",);
    }
    ...
    public function assetHodled(string $policyId, string $assetId, string $stakeKey): bool {
        $assetAddresses = $this->call(self::HTTP_REQUEST_GET, "assets/{$policyId}{$assetId}/addresses",);
        ...
        $addressInfo = $this->call(self::HTTP_REQUEST_GET, "addresses/{$firstAssetAddress}",);
    ...
    private function buildEndpoint(string $requestUri): string {
        return sprintf('https://cardano-%s.blockfrost.io/api/v0/%s', env('CARDANO_NETWORK'), $requestUri);
    }
}
